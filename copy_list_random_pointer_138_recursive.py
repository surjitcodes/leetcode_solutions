...
class Solution(object):
    ...
    def copyRandomList(self, head):
        ...
        # return clone of node if already processed
        if head in self.cloning_dictionary:
            return self.cloning_dictionary[head]

        # create new clone with same val as old clone
        clone = Node(head.val, None, None)

        # save relation of old and new
        self.cloning_dictionary[head] = clone

        # Recursively copy next pointers of all nodes
        clone.next = self.copyRandomList(head.next)
        # Recursively copy random pointers of all nodes
