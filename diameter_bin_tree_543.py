...
class Solution:
    def diameterOfBinaryTree(self, root: TreeNode) -> int:
        ...
        def longest_path(node):      # DFS
            if not node:     # length is 0
                return 0

            nonlocal diameter

            # recursively find the lengths of two sides.
            longest_left_path = longest_path(node.left)
            longest_right_path = longest_path(node.right)

            # update dia if longest_left_path + longest_right_path is larger
            diameter = max(diameter, longest_left_path + longest_right_path)

            # return the longer of longest_left_path and longest_right_path
...
root = get_tree_root_from_bfs([2, 4, 6, None, None, 1, 5, 3, None, None, 9, 8, None, 7])
# root = get_tree_root_from_bfs([-10, 9, 20, None, None, 15, 7, 8, No])
show_tree_from_root(root)
sol = Solution()
