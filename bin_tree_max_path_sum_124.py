...
class Solution:
    def max_path_sum(self, root: TreeNode) -> int:
        ...
        def gain_from(node: TreeNode) -> int:
            nonlocal max_sum

            if not node:
                return 0

            # Gains from two subtrees. Ignore if negative
            gain_left = max(gain_from(node.left), 0)
            gain_right = max(gain_from(node.right), 0)

            path_sum = node.val + gain_left + gain_right
            max_sum = max(max_sum, path_sum)

            # return gain from the node to its parent
...
root = get_tree_root_from_bfs([-2, -3, -4, -5, -6, None, -7, -3, 40])
show_tree_from_root(root)
sol = Solution()
output = sol.max_path_sum(root)
print(output)
'''
